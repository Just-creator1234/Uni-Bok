...
import UserInsights from "./UserInsights";
import * as XLSX from "xlsx";
import { updateUserProfile } from "@/app/actions/editUser";

export default function UserTable({ users, currentUserRole }) {
  const [filterRegistered, setFilterRegistered] = useState("");
  const [selectedUser, setSelectedUser] = useState(null);
  const [isPending, startTransition] = useTransition();
  const [searchTerm, setSearchTerm] = useState("");
  ...
  const handleExportFiltered = () => {
    const data = filteredUsers.map(
      ({ name, email, indexNo, level, semester, role }) => ({
        Name: name,
        Email: email,
        "Index No": indexNo || "-",
        Level: level,
  ...
  const handleExportAll = () => {
    const data = users.map(({ name, email, indexNo, level, semester, role }) => ({
      Name: name,
      Email: email,
      "Index No": indexNo || "-",
      Level: level,
  ...
  // Determine if current user can edit a specific user
  const canEditUser = (user) => {
    // If current user is SUPER_ADMIN, can edit everyone
    if (currentUserRole === "SUPER_ADMIN") return true;
    
    // If current user is ADMIN, can only edit STUDENTS
    if (currentUserRole === "ADMIN" && user.role === "STUDENT") return true;
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="text-sm text-gray-400 italic mb-2 block md:hidden text-center">
        Better view on PC. Swipe left/right to see more columns →
      </div>
      ...
      {editUser && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-[25rem] shadow-lg">
            <h2 className="text-lg font-bold mb-4 text-blue-900">
              {editUser.role === "STUDENT" ? "Edit Student" : "Edit User"}
            </h2>
            <div className="space-y-3">
